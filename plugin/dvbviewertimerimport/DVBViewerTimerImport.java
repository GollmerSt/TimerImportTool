...
public class DVBViewerTimerImport extends Plugin implements DVBViewerProvider {
  ...
  private static final long searchIntervallOrg = 15 * 60 * 1000; // search
                                                                 // Intervall of
                                                                 // 1/4 hours
                                                                 // before
                                                                 // original
                                                                 ...
                                                                 // original end
                                                                 // of programm
  private static final long searchIntervallReal = 3 * 60 * 60 * 1000; // search
                                                                      // Intervall
                                                                      // of 3
                                                                      // hours
                                                                      // before
   ...
   * 
   * @param settings
   *          The settings for this plugin (May be empty).
   */
  @Override
  ...
  public static ProviderChannel<String>[] getTVBChannelNames() {
    ...
    ProviderChannel<String>[] newInstance = (ProviderChannel<String>[]) Array.newInstance(new ProviderChannel<String>(
        "", "").getClass(), channels.length);
    ...
    for (Map.Entry<String, List<String>> mapEntry : checkMap.entrySet()) {
      ...
      DVBViewerTimerImport.plugin.tvbChannelNames[++idx] = new ProviderChannel<String>(mapEntry.getKey(), mapEntry
          .getValue().get(0));
      if (list.size() > 1) {
        int cnt = 1;
        for (cnt = 1; cnt < list.size(); ++cnt)
          DVBViewerTimerImport.plugin.tvbChannelNames[++idx] = new ProviderChannel<String>(mapEntry.getKey() + "("
              + Integer.toString(cnt) + ")", list.get(cnt));
      }
    }
    return DVBViewerTimerImport.plugin.tvbChannelNames;
  }
  ...
  /**
   * @param e
   *          Throwable of the last exception/error ....
   */
  ...
  public PluginInfo getInfo() {
    if (pluginInfo == null)
      pluginInfo = new PluginInfo(DVBViewerTimerImport.class, ResourceManager.msg("PLUGIN_NAME"),
          ResourceManager.msg("DESCRIPTION"), "Gollmer, Stefan");
    return pluginInfo;
  }

  // TODO hier kann man für die verschiedenen Provuder unterschiedliche Icons
  // definieren
  @Override
  ...
  private class DVBViewerChannelChooseAction extends AbstractAction {
    ...
    public void actionPerformed(ActionEvent e) {
      ...
      ChannelSet dvbChannel = null;

      try {
        dvbChannel = dvbViewer.getDVBViewerChannelSet(provider.getID(), tvBChannel.getName());
      } catch (ErrorClass e1) {
        errorMessage(e1);
        return;
      }

      dvbViewer.startDVBViewerAndSelectChannel(dvbChannel.getDVBViewerChannel());
    }
  }
  ...
  /**
   * @param program
   *          program which will seach in the DVBViewer recording list
  ...
  public DVBViewerEntry findProgram(final Program program) {
    ...
    for (DVBViewerEntry co : this.control.getDVBViewer().getRecordEntries()) {
      if (co.getTvBrowserID() != null && co.getTvBrowserID().equals(program.getUniqueID()) && co.isProgramEntry()) {
        entry = co;
        break;
      }
    }
  ...
  private static long[] calcRecordTimes(final Program program) {
    ...
    calendar.set(d.getYear(), d.getMonth() - 1, d.getDayOfMonth(), program.getHours(), program.getMinutes());
    ...
    if (length >= 0)
      ...
      for (int t = 0; t < 2 && !finished; t++) {
        ...
        while (pIt.hasNext()) {
          ...
          Date dd = p.getDate();
          calendar.clear();
          calendar.set(dd.getYear(), dd.getMonth() - 1, dd.getDayOfMonth(), p.getHours(), p.getMinutes());
  ...
  public Program searchBestFit(DVBViewerEntry entry) {
    ...
    for (Iterator<Program> it = possibilities.iterator(); it.hasNext();) {
      ...
      Date dd = pgm.getDate();
      calendar.clear();
      calendar.set(dd.getYear(), dd.getMonth() - 1, dd.getDayOfMonth(), pgm.getHours(), pgm.getMinutes());
  ...
  private static class SearchEntryBrowser extends SearchEntry {
    ...
    final long start;
    final long end;
    final String dvbViewerChannel;

    public SearchEntryBrowser(Program pgm, String dvbViewerChannel) {
      long[] times = DVBViewerTimerImport.calcRecordTimes(pgm);
      this.program = pgm;
      this.start = times[0];
      this.end = times[1];
      this.dvbViewerChannel = dvbViewerChannel;
    }
    ...
    @Override
    public String getChannelID() {
      if (this.program.getChannel() == null)
        return null;
      else
  ...
  public static class SearchEntryViewer extends DVBViewerTimerImport.SearchEntry {
    ...
    @Override
    public String getChannelID() {
      if (this.entry.getChannelSet() == null)
        return null;
      else
  ...
  void updateForeignEntries() {
    ...
    for (DVBViewerEntry dE : dvbViewer.getRecordEntries()) {
      ...
      for (Channel channel : this.getChannels(dE)) {
        ...
        for (int i = 0; i < endCnt; ++i) {
          ...
          for (; it.hasNext();) {
            Program pgm = it.next();
            browserMap.put(pgm.getUniqueID(), new SearchEntryBrowser(pgm, dE.getChannel()));
          }
        }
      }
    }
    ...
    for (Iterator<SearchEntry> itD = dvbList.iterator(); itD.hasNext();) {
      ...
      Helper.SearchBiDirectional<SearchEntry>.Result list = searchBi.new Result(true,
          d.searchSurroundedEntries(browserList));
      ...
      // find the best match of title
      {
        list = searchBi.searchBiDirectional(d, list.get(), dvbList, new Helper.SearchAlgorithm<SearchEntry>() {
          ...
          @Override
          public ArrayList<SearchEntry> execute(SearchEntry entry, ArrayList<SearchEntry> entries) {
            return Helper.getTheBestChoices(entry.getTitle(), entries, 2, 3, new Function(), new Function() {
              @Override
              public int arrayIntToInt(final ArrayList<Integer> list, final int integer, final String search,
                  final String array) {
        ...
        list = searchBi.searchBiDirectional(d, list.get(), browserList, new Helper.SearchAlgorithm<SearchEntry>() {
          ...
          public ArrayList<SearchEntry> execute(SearchEntry entry, ArrayList<SearchEntry> entries) {
            ...
            for (SearchEntry e : entries) {
              ...
              diff += diff1 / 2; // the weight of the post offset is lower than
                                 // the pre offset
              if (diff < minDiff) {
                choices.clear();
                choices.add(e);
  ...
  public Collection<Channel> getChannels(final DVBViewerEntry entry) {
    Collection<Channel> result = new ArrayList<Channel>();
    if (entry.getChannelSet() != null) {
      String uniqueName = (String) entry.getChannelSet().getChannel(DVBViewerTimerImport.providerID).getIDKey();
      result.add(DVBViewerTimerImport.getUniqueAssignmentMap().get(uniqueName));
    } else {
      for (ChannelSet cs : this.dvbViewer.getChannelSetListByDvbViewerChannel(entry.getChannel())) {
        String uniqueName = (String) cs.getChannel(DVBViewerTimerImport.providerID).getIDKey();
        result.add(DVBViewerTimerImport.getUniqueAssignmentMap().get(uniqueName));
      }
    }
