...
public class TimersDialog extends JDialog {
	...
	private static ImageIcon programIcon = ResourceManager.createImageIcon("icons/dvbViewer Programm16.png",
			"DVBViewer icon");
	...
	static class MyTreeTable extends JTreeTable {
		...
		static class MyTreeRenderer extends DefaultTreeCellRenderer {
			...
			private ImageIcon activeTimerIcon = ResourceManager.createImageIcon("icons/dvbViewer Timer14.png",
					"Timer icon");
			private ImageIcon disabledTimerIcon = ResourceManager
					.createImageIcon("icons/dvbViewer TimerDisabled 14.png", "Timer icon deleted");
			private ImageIcon filterTimerIcon = ResourceManager.createImageIcon("icons/dvbViewer TimerFilter 14.png",
					"Filter icon");
			private ImageIcon recordingIcon = ResourceManager.createImageIcon("icons/dvbViewer Recording 14.png",
					"Recording icon");

			public MyTreeRenderer() {
				super();

				this.setLeafIcon(this.activeTimerIcon);
	...
	public void init() {
		...
		Provider.updateRecordingsAllProviders(this.control.getDVBViewer().getRecordEntries());
		...
		MyTreeTable.MyTreeRenderer renderer = new MyTreeTable.MyTreeRenderer();
	...
	class MyTableMouseListener implements MouseListener {
		...
		private void mayShowTablePopUp(MouseEvent evt) {
			if (evt.getButton() == MouseEvent.BUTTON3 && evt.isPopupTrigger()) {
				int row = TimersDialog.this.recordingTable.rowAtPoint(evt.getPoint());
				if (!TimersDialog.this.recordingTable.isRowSelected(row))
					TimersDialog.this.recordingTable.changeSelection(row, 0, false, false);
				this.rightButtonPressed = true;
				TimersDialog.this.tablePopUp.show(evt.getComponent(), evt.getX(), evt.getY());
	...
	private class ButtonsPressed implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			TimersDialog.this.recordingTable.editingStopped(null);
			JButton source = (JButton) e.getSource();

			TreeTableCellRenderer jTree = (TreeTableCellRenderer) TimersDialog.this.recordingTable.getTree();
			TimersTreeTableModel treeModel = (TimersTreeTableModel) jTree.getModel();

			if (source == TimersDialog.this.okButton) {
				if (treeModel.isChanged()) {
					try {
						TimersDialog.this.control.getDVBViewer().setDVBViewerTimers();
					} catch (InterruptedException e1) {
						// TODO Auto-generated catch block
						e1.printStackTrace();
					}
					TimersDialog.this.control.getDVBViewer().updateDVBViewer(false);
					TimersDialog.this.control.getDVBViewer().writeXML();
					Provider.updateRecordingsAllProviders(TimersDialog.this.control.getDVBViewer().getRecordEntries());
					treeModel.setIsChanged(false);
				}
				dispose();
			} else if (source == TimersDialog.this.cancelButton) {
				TimersDialog.this.control.getDVBViewer().resetRecordEntries();
				treeModel.setIsChanged(false);
				dispose();
			} else if (source == TimersDialog.this.applyButton) {
				TimersDialog.this.recordingTable.editingStopped(null);
				if (treeModel.isChanged()) {
					try {
						TimersDialog.this.control.getDVBViewer().setDVBViewerTimers();
			...
			} else if (source == TimersDialog.this.reloadButton) {
				TimersDialog.this.control.getDVBViewer().updateDVBViewer(true);
				Provider.updateRecordingsAllProviders(TimersDialog.this.control.getDVBViewer().getRecordEntries());
				treeModel.setIsChanged(false);
