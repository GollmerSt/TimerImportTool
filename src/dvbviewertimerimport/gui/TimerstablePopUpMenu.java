...
public class TimerstablePopUpMenu extends JPopupMenu
{
	...
	class MyActionListener implements ActionListener
	{
		...
		public void actionPerformed(ActionEvent evt) {
			...
			DVBViewerEntry [] entries = table.getSelectedEntries() ;
			if ( entries.length == 0 )
				return ;
			...
			for ( MenuEnum e : MenuEnum.values() )
			{
				if ( item == e.getItem() )
				{
					e.method.choosedListener( control.getDVBViewer(), entries ) ;
					control.getDVBViewer().reworkMergeElements() ;
					TreeTableCellRenderer jTree = (TreeTableCellRenderer) table.getTree() ;
